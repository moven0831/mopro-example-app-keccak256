...
import keccakInput from '../test-vectors/noir/keccak256_input.json';
...
// Function to fetch the keccak input JSON
async function getKeccakInput() {
  showAppend("logs", "Fetching keccak input JSON... ⏳");
  // Path relative to web/index.js: ../test-vectors/noir/keccak_input.json
  // const response = await fetch('../test-vectors/noir/keccak_input.json');
  // if (!response.ok) {
  //   showAppend("logs", `Failed to fetch keccak input JSON: ${response.statusText} 💔`);
  //   throw new Error(`Failed to fetch keccak input JSON: ${response.statusText}`);
  // }
  // const localKeccakInput = await response.json(); // Renamed to avoid conflict if needed
  showAppend("logs", "Fetched keccak input JSON... ✅");
  return keccakInput; // Use the statically imported keccakInput
}
...
document.getElementById("submit").addEventListener("click", async () => {
  try {
    ...
    const inputs = await getKeccakInput();
